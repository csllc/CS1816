...
export default class DongleTransaction {
  ...
  constructor(options) {
    ...
    if('function' === typeof(logger.child)) {
      logger = logger.child({ component: 'DongleTransaction' });
    }
  ...
  handleResponse(response) {
    ...
    if(this.options.client && !this.isCancelled()) {
  ...
  handleError(error) {
    ...
    logger.log('info', `handleError ${this.failures}`);
    this.stopTimeout();
