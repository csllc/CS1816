...
class IpHeader {
  ...
  validate(transaction) {
    ...
    if(this.version !== 0) {
      message =
        `Invalid version in response header. Expected: 0, got: ${this.version}`;
...
export default class DongleTransport {
  ...
  constructor(connection, options) {
    ...
    if('function' === typeof(logger.child)) {
      logger = logger.child({ component: 'DongleTransport' });
    }
  ...
  sendRequest(transaction) {
    ...
    logger.log('info', `sendRequest ${id}`);
   ...
   */
  onData(data) {
    logger.log('silly', `onData ${data}, ${this.reader.length} ${this.reader}`);
   ...
   */
  handleFrameData() {
    logger.log('debug', `handleFrameData ${this.header.id}`);
   ...
   */
  skipResponseData() {
    logger.log('debug', 'skipResponseData');
