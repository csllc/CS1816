...
export default class DeviceInterface {
  ...
  constructor(bytes) {
    ...
    // uint8_t mode;       // a SYSTEM_MODE
    // uint8_t protocol;   // a SYSTEM_PROTOCOL
    // uint8_t config;     // meaning depends on the mode (eg might be a baud rate enumeration)
    // uint8_t status;     // e.g communication interface up/down
    if(bytes) {
      this.set(bytes);
    } else {
      this.mode = '';
