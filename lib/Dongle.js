 ...
 *
 * The constructor accepts a 'Ble' object that we rely on to communicate with
 * the BLE hardware.
 *
 * @type       {Class}
 */
...
const COMMAND_OP_RESET = 0xFB;

// Constants used by specific op codes (sub-commands)
const COMMAND_OP_RESET_DATA = 0x01;
...
const MAX_WATCHERS = 15; // as per dongle firmware 1.11.0
const MAX_SUPERWATCHERS = 25;

// we will ask the Ble manager for all the 'chars of interest'
...
const DEFAULT_OPTIONS = {
  ...
  // the default was determined based on the dongle's internal timeout for a max-
  // length read or write operation.  It is much cooler if our timeout is longer
  // than the dongle's, because otherwise the dongle might respond after we have
  // already given up and moved on.  Things *should* recover if that isn't the
  // case, but we'd rather not have to deal with that
  defaultTimeout: 6500,
...
export default class Dongle {
  ...
  constructor(ble, options) {
    ...
    logger.log('debug', 'Options:', this.options);
  ...
  destroyAllTransactions() {
    logger.log('warn', 'DEPRECATED; use cancelTransactions and handle cancellations cleanly');
  ...
  async readString(characteristic) {
    logger.log('debug', 'readStringCharacteristic', characteristic.uuid);
  ...
  async onDisconnect(cb) {
    this.ble.onDisconnect(this.peripheral, () => cb(this));
  }
  ...
  async configureI2c(options) {
    logger.log('debug', 'configureI2c %O', options);
    let response = await this.execute(this.ID, [FN_COMMAND, COMMAND_OP_CONFIG, 0x04, 0x01, 0x00, 0x00], options);
    if(response[2] !== 0) {
      throw new Error('Failed to configure I2C')
    }
  }
  ...
  async updateSuperWatcher(options) {
    ...
    let addresses = [];
    let unit;
    options = Object.assign({ packed: false }, options);
    ...
    if(addresses.length > 0) {
      ...
      let opcode = (options.packed) ? COMMAND_OP_SUPERWATCH_PACKED : COMMAND_OP_SET_SUPERWATCHER;
  ...
  /**
   * Issue the reset command to the specified device
   *
   * @param      {Number}   unit     Device which we want to reset
   * @param      {Number}   options  message options
   */
  async reset(unit, options) {
    logger.log('debug', 'reset %d %O', unit, options);
    let result = await this.execute(unit, [FN_COMMAND, COMMAND_OP_RESET, COMMAND_OP_RESET_DATA], options);
  ...
  executeQueuedTransactions() {
    logger.log('debug', 'executeQueuedTransactions %d', this.transactionQueue.length);

    while(this.transactionQueue.length > 0 &&
      this.executingRequests < this.options.maxConcurrentRequests) {
      var transaction = this.transactionQueue.shift();

      // if it's cancelled we just ignore it and it will die.  otherwise start it
  ...
  // Event from ble-manager containing a transparent uart
  // notification
  onData(data) {
  ...
  onSuperWatcherNotify(data) {
    ...
    if(data.value.length % 3 === 0) {
      ...
      for (let i = 0; i < data.value.length; i += 3) {
        let address = data.value[i] * 256 + data.value[i + 1];
        data.value = [data.value[i + 2]];
        this.superWatchers.forEach(watcher => {
