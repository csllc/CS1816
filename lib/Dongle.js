...
import DongleTransport from './DongleTransport';
import DongleTransaction from './DongleTransaction';
import { DONGLE_UUIDS } from './DongleUUIDs';
import Logger from './CsLogger';
...
const FN_WRITE_VERIFY_MEMORY = 0x48;
...
class Watcher {
  ...
  handleUpdate(data) {
    //console.log('watcher handleupdate', data);
    if(data.value.length === this.length) {
      if('function' === typeof this.cb) {
        this.cb(data.value, this.address, this.unit);
      }
    } else {
      console.error('Watcher update with incorrect length!', data.value.length, this.length);
    }
  }
...
export default class Dongle {
  ...
  async readDongleInfo() {
    ...
    .then(() => logger.timeEnd('readDongleInfo'));
  }
   ...
   * @return     {Promise}  { resolves when complete }
   */
  writeObject(unit, objectId, data, options) {
    logger.info('writeObject', unit, objectId, options);
    data = data || [];
    return this.execute(unit, [FN_WRITE_OBJECT, objectId, ...data], options);
  }
  ...
  onData(data) {
    ...
    switch (data.characteristic) {
      ...
      case DONGLE_UUIDS.uuidSuperWatcher:
        logger.info('Notify superwatcher', data);
        break;

      case DONGLE_UUIDS.uuidRx:
        logger.info('Notify RX', data);
