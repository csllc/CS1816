...
export default class Dongle {
  ...
  destroyAllTransactions() {
    ...
    this.transport.destroyAllTransactions();
  }
  ...
  async write(bytes) {
    ...
    //
    this.ble.write(this.peripheral, DONGLE_UUIDS.uuidUartService, DONGLE_UUIDS.uuidTx, bytes);
  }
  ...
  async readDongleInfo() {
    ...
    return Promise.all(todo).then(results => {
  ...
  async watch(unit, address, length, cb) {
    ...
    await this.ble.subscribe(
      this.peripheral,
      DONGLE_UUIDS.uuidControllerService,
      DONGLE_UUIDS.uuidStatus[slot],
      watcher.handleUpdate.bind(watcher),
    );
  ...
  //subscribes to super watcher characteristic
  async subscribeToSuperWatcher() {
    await this.ble.subscribe(
      this.peripheral,
      DONGLE_UUIDS.uuidControllerService,
      DONGLE_UUIDS.uuidSuperWatcher,
      this.onSuperWatcherNotify.bind(this),
    );
  }
  ...
  async readMemory(unit, address, length, options) {
    ...
    let data = await this.execute(
      unit,
      [FN_READ_MEMORY, (address >> 8) & 0xff, address & 0xff, length],
      options,
    );
  ...
  writeMemory(unit, address, data, options) {
    ...
    return this.execute(
      unit,
      [FN_WRITE_MEMORY, (address >> 8) & 0xff, address & 0xff, ...data],
      options,
    );
  }
  ...
  writeVerifyMemory(unit, address, data, options) {
    ...
    return this.execute(
      unit,
      [FN_WRITE_VERIFY_MEMORY, (address >> 8) & 0xff, address & 0xff, ...data],
      options,
    );
  }
  ...
  async writeObject(unit, objectId, data, options) {
    ...
    let result = await this.execute(
      unit,
      [FN_WRITE_OBJECT, objectId, data.length, ...data],
      options,
    );
